# frozen_string_literal: true

require 'spec_helper'

describe ::CA::SSH::Signatory do
  describe '#sign' do
    let(:inputs) do
      ::CA::SigningInputs.new(kind, params, role, ca_resource, env)
    end
  
    let(:kind) { "ssh" }
    let(:params) do
      {
        public_key: public_key,
        principals: principals
      }
    end
    let(:role) { double("role", id: "rspec:user:alice") }
    let(:ca_resource) { double("ca_resource", account: "rspec") }
    let(:env) { double("env") }

    let(:public_key) { "ssh-rsa " }
    
    let(:public_key_format) { nil }
    let(:principals) { double("principals") }

    let(:private_key_variable) do
      double("private_key", secret: double("secret", value: private_key))
    end

    let(:private_key_id) { "conjur/ca/ssh/private-key"}
    let(:private_key_password_id) { nil }

    let(:private_key) do
      <<~KEY
        -----BEGIN RSA PRIVATE KEY-----
        
        -----END RSA PRIVATE KEY-----
      KEY
    end

    let(:private_key_password) { nil }

    subject { ::CA::SSH::Signatory.new(inputs).() }

    before do
      allow(Resource)
        .to receive(:[])
        .with("rspec:variable:#{private_key_id}")
        .and_return(private_key_variable)

      allow(ca_resource).to receive(:annotation).with("ca/max-ttl").and_return('P1D')
      allow(ca_resource).to receive(:annotation).with("ca/private-key-password").and_return(private_key_password_id)
      allow(ca_resource).to receive(:annotation).with("ca/private-key").and_return(private_key_id)
    end

    context "when all of the inputs are valid" do
      it "returns a signed certificate" do
        expect(subject).to be_a(Net::SSH::Authentication::Certificate)
      end
    end
  end
end
