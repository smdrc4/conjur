...
describe ::CA::X509::Renderer do
  ...
  describe '#render' do
    ...
    subject { ::CA::X509::Renderer.new(certificate).() }

    it "renders an x.509 certificate to PEM format" do
      expect(subject.body).to eq(certificate_formatted)
    end

    it "returns a content type of 'application/x-pem-file" do
      expect(subject.content_type).to eq('application/x-pem-file')
    end
  end
end
