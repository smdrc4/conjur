...
describe ::CA::X509::Signatory do
  describe '#sign' do
    let(:inputs) do
      ::CA::SigningInputs.new(kind, params, role, ca_resource, env)
    end
    ...
    let(:role) { double("role", id: "rspec:user:alice", account: "rspec", kind: "host", identifier: "alice") }
    let(:ca_resource) { double("ca_resource", account: "rspec", identifier: "conjur/ca/x509") }
    let(:env) { double("env") }
    ...
    subject { ::CA::X509::Signatory.new(inputs).() }
    ...
    before do
      ...
      allow(ca_resource).to receive(:annotation).with("ca/max-ttl").and_return('P1D')
      allow(ca_resource).to receive(:annotation).with("ca/private-key-password").and_return(private_key_password_id)
      allow(ca_resource).to receive(:annotation).with("ca/private-key").and_return(private_key_id)
      allow(ca_resource).to receive(:annotation).with("ca/certificate").and_return(cert_chain_id)
    end

    context "when all of the inputs are valid" do
      it "returns a signed certificate" do
        expect(subject).to be_a(OpenSSL::X509::Certificate)
      end
    end
  end
end
